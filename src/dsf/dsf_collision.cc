...
inline void DcfImpDiff(DSF* dsf1, const Vector3d& pos1, const Matrix3d& R1,
                       DSF* dsf2, const Vector3d& pos2, const Matrix3d& R2,
                       const Vector4d& var, DCF& dcf) {
  Vector3d s1, s2;
  Matrix3d ds1dx, ds2dx;
  Matrix<double, 3, 6> ds1dq, ds2dq;
  dsf1->SupportFunc(var.head<3>(), pos1, R1, s1, ds1dx, ds1dq);
  dsf2->SupportFunc(-var.head<3>(), pos2, R2, s2, ds2dx, ds2dq);

  const Vector3d q_bar{pos1 - pos2};
  Matrix4d dedvar;
  dedvar.block<3, 3>(0, 0) = var(3) * (ds1dx + ds2dx);
  dedvar.block<3, 1>(0, 3) = s1 - s2 - q_bar;
  dedvar.block<1, 3>(3, 0) = 2.0 * var.head<3>().transpose();
  dedvar(3, 3) = 0.0;
  Matrix<double, 4, 12> dedq{Matrix<double, 4, 12>::Zero()};
  dedq.block<3, 6>(0, 0) = var(3) * ds1dq;
  dedq.block<3, 3>(0, 0) += (1.0 - var(3)) * Matrix3d::Identity();
  dedq.block<3, 6>(0, 6) = -var(3) * ds2dq;
  dedq.block<3, 3>(0, 6) -= (1.0 - var(3)) * Matrix3d::Identity();
  const Matrix<double, 3, 12> dxdq{
  ...
  dcf.dnormal =
      (Matrix3d::Identity() - dcf.normal * dcf.normal.transpose()) * dxdq;
...
double GrowthDistance(DSF* dsf1, const Matrix4d& tf1, DSF* dsf2,
  ...
  const Vector3d pos1{tf1.block<3, 1>(0, 3)}, pos2{tf2.block<3, 1>(0, 3)};
  const Matrix3d R1{tf1.block<3, 3>(0, 0)}, R2{tf2.block<3, 3>(0, 0)};
  ...
  const Vector3d u0(1.0, 1.0, 1.0);
  Matrix3d W_sim, Winv_sim;
  Vector3d c_sim, x;
  Matrix<double, 3, 4> V_sim;
  const double small{1e-3};
  V_sim.col(0) = small * Vector3d(1.0, 0.0, -0.5);
  V_sim.col(1) = small * Vector3d(-0.5, 0.5, -0.5);
  V_sim.col(2) = small * Vector3d(-0.5, -0.5, -0.5);
  V_sim.col(3) = small * Vector3d(0.0, 0.0, 1.0);
  ...
  while (true) {
    ...
    if (out.iter <= 4) {
      ...
      if (out.iter == 4) {
        dsf1->SupportFunc(x, pos1, R1, s1, ds1dx);
        dsf2->SupportFunc(-x, pos2, R2, s2, ds2dx);
      } else {
        dsf1->SupportFunc(x, pos1, R1, s1);
        dsf2->SupportFunc(-x, pos2, R2, s2);
      }
...
SolutionError ComputeSolutionError(DSF* dsf1, const Matrix4d& tf1, DSF* dsf2,
  ...
  const Vector3d normal{dcf.normal}, q_bar{pos1 - pos2};

  SolutionError err = SolutionError{};
  if (out.status == SolutionStatus::CoincidentCenters) return err;

  Vector3d s1, s2;
  dsf1->SupportFunc(normal, pos1, R1, s1);
  dsf2->SupportFunc(-normal, pos2, R2, s2);
