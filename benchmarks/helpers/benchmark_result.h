...
struct BenchmarkResultArray {
  ...
  std::vector<double> prim_feas_errs;
  std::vector<double> dual_feas_errs;
...
inline void BenchmarkResultArray::AddResult(const BenchmarkResult& res) {
  if (idx < size) {
    ...
    prim_feas_errs[idx] = res.prim_feas_err;
    dual_feas_errs[idx] = res.dual_feas_err;
    iters[idx] = res.iter;
...
inline double BenchmarkResultArray::MaxPrimalFeasibilityError() const {
  if (idx == 0) return 0.0;
  return *std::max_element(prim_feas_errs.begin(), prim_feas_errs.end());
}

inline double BenchmarkResultArray::MaxDualFeasibilityError() const {
  if (idx == 0) return 0.0;
  return *std::max_element(dual_feas_errs.begin(), dual_feas_errs.end());
}
...
inline void BenchmarkResultArray::PrintStatistics() const {
  ...
  std::cout << "Max. prim feas err (m): " << MaxPrimalFeasibilityError()
            << std::endl;
  std::cout << "Max. dual feas err    : " << MaxDualFeasibilityError()
            << std::endl;
