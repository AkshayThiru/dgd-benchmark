#ifndef IE_INTERNAL_EXPANDING_H_
#define IE_INTERNAL_EXPANDING_H_
...
struct Settings {
  int max_iter{100};
  double min_center_dist{dgd::kEpsSqrt};
  double tol{dgd::kEpsSqrt};
...
// Solver output.
struct Output {
  dgd::Matf<3, 4> s1_{dgd::Matf<3, 4>::Zero()};
  dgd::Matf<3, 4> s2_{dgd::Matf<3, 4>::Zero()};
  dgd::Vecf<4> bc_{dgd::Vecf<4>::Zero()};

  dgd::SupportFunctionHint<3> hint1_{};
  dgd::SupportFunctionHint<3> hint2_{};

  dgd::Vec3f normal{dgd::Vec3f::Zero()};
  dgd::Vec3f z1{dgd::Vec3f::Zero()};
  dgd::Vec3f z2{dgd::Vec3f::Zero()};
  double growth_dist{0.0};

  int iter{0};
  SolutionStatus status{SolutionStatus::Optimal};
};

// Growth distance function.
template <class C1, class C2>
double GrowthDistance(const C1* set1, const dgd::Transform3f& tf1,
                      const C2* set2, const dgd::Transform3f& tf2,
                      const Settings& settings, Output& out) {
  static_assert((C1::Dimension() == 3) && (C2::Dimension() == 3),
                "Convex sets are not three-dimensional");

  out.hint2_.n_prev = out.hint1_.n_prev = dgd::Vec3f::Zero();
  out.iter = 0;

  // Check center distance.
  const dgd::Vec3f p1{tf1.block<3, 1>(0, 3)}, p2{tf2.block<3, 1>(0, 3)};
  const dgd::Vec3f p12{p1 - p2};
  const double cdist{p12.norm()};
  if (cdist < settings.min_center_dist) {
    out.normal = dgd::Vec3f::Zero();
    out.z1 = p1;
    out.z2 = p2;
    out.growth_dist = 0.0;
  ...
  const dgd::Rot3f rot1{tf1.block<3, 3>(0, 0)}, rot2{tf2.block<3, 3>(0, 0)};

  // Simplex variables.
  const dgd::Vec3f u0{dgd::Vec3f::Ones()};
  dgd::Matf<3, 3> W, Winv;
  dgd::Vec3f cone_coeff;
  dgd::Vec3f sp1, sp2, sp;
...
#endif
  V.col(0) = simplex_size * dgd::Vec3f(0.8, 0.0, -0.5);
  V.col(1) = simplex_size * dgd::Vec3f(-0.5, 0.5, -0.5);
  V.col(2) = simplex_size * dgd::Vec3f(-0.5, -0.5, -0.5);
  V.col(3) = simplex_size * dgd::Vec3f(0.0, 0.0, 1.0);
...
inline SolutionError ComputeSolutionError(const dgd::ConvexSet<3>* set1,
                                          const dgd::Transform3f& tf1,
                                          const dgd::ConvexSet<3>* set2,
                                          const dgd::Transform3f& tf2,
                                          const Output& out) {
  const dgd::Vec3f p1{tf1.block<3, 1>(0, 3)}, p2{tf2.block<3, 1>(0, 3)};
  const dgd::Rot3f rot1{tf1.block<3, 3>(0, 0)}, rot2{tf2.block<3, 3>(0, 0)};
  const dgd::Vec3f cp1{p1 + out.growth_dist * (out.z1 - p1)};
  const dgd::Vec3f cp2{p2 + out.growth_dist * (out.z2 - p2)};
...
#endif  // IE_INTERNAL_EXPANDING_H_
